...
type AppFeature = { key: string; label: string; is_active: boolean; updated_at?: string };
type PremiumFeature = { key: string; label_ar?: string; label_fr?: string; is_active: boolean; sort_order?: number };
...
export default function FeatureFlagsPage() {
  ...
  const load = async () => {
    ...
    try {
      const [app, prem] = await Promise.all([
        api.adminAppFeatures() as Promise<AppFeature[]>,
        api.adminPremiumFeatures() as Promise<PremiumFeature[]>,
      ]);
  ...
  const setApp = async (key: string, next: boolean) => {
    ...
    try {
      await api.setAdminAppFeature(key, next);
    } catch (e: any) {
      await load();
  ...
  const setPremium = async (key: string, next: boolean) => {
    ...
    try {
      await api.setAdminPremiumFeature(key, next);
    } catch (e: any) {
      await load();
      alert(e.message || 'Erreur');
    }
  ...
  const disableAll = async () => {
    ...
    try {
      await Promise.all([api.disableAllAppFeatures(), api.disableAllPremiumFeatures()]);
      await load();
    } catch (e: any) {
      alert(e.message || 'Erreur');
    }
