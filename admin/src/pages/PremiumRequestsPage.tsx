...
const apiGetRequests = (status: string, page: number) =>
  (api as any).request
    ? (api as any).request('GET', `/billing/feature-requests?status=${status}&page=${page}&limit=20`)
    : fetch(`/api/v1/billing/feature-requests?status=${status}&page=${page}&limit=20`, {
  ...
  const handleConfirm = async (note: string) => {
    ...
    try {
      if (actionType === 'approve') {
        await api.approveFeatureRequest(actionTarget.id, note);
        showToast('✅ تم قبول الطلب وتفعيل الوصول');
      } else {
        await api.rejectFeatureRequest(actionTarget.id, note);
