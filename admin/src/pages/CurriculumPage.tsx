...
function FacultiesTab() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.createFaculty({ ...form, sort_order: Number(form.sort_order) });
  ...
  const handleEdit = async (faculty: Faculty) => {
    if (editId === faculty.id) {
      // Save
      try {
        await api.updateFaculty(faculty.id, {
  ...
  const handleToggle = async (f: Faculty) => {
    try {
      await api.updateFaculty(f.id, { is_active: !f.is_active });
      load();
    } catch {
      alert('Erreur');
    }
...
function FacultyDetailModal({ faculty, onClose }: { faculty: Faculty; onClose: () => void }) {
  ...
  useEffect(() => {
    setLoading(true);
    api.adminSubjects(faculty.slug)
...
function SubjectsTab() {
  ...
  const loadSubjects = () => {
    setLoading(true);
    api.adminSubjects(filterFaculty || undefined, filterYear ? Number(filterYear) : undefined)
  ...
  const handleToggle = async (s: Subject) => {
    try {
      await api.updateSubject(s.id, { is_active: !s.is_active });
      loadSubjects();
    } catch {
      alert('Erreur');
    }
