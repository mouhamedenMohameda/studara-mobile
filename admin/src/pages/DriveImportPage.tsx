...
export default function DriveImportPage() {
  ...
  useEffect(() => {
    api.adminFaculties()
      .then((list: Faculty[]) => {
        const active = (list || []).filter((f: Faculty) => f.is_active);
        setFaculties(active);
        if (active.length > 0) {
      ...
      .catch(() => {});
    api.listDriveJobs().then((data: ImportJob[]) => setHistory(data || [])).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        const j: ImportJob = await api.getDriveJob(currentJobId!);
        setJob(j);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setJob(null);
    try {
      const r = await api.startDriveImport(driveUrl, faculty, year, university);
