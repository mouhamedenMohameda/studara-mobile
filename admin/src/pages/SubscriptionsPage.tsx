...
export default function SubscriptionsPage() {
  ...
  const searchUsers = useCallback(async () => {
    ...
    setSearching(true);
    try {
      const data = await api.users(1, query) as User[];
      setUsers(data);
    } catch {
      setUsers([]);
    } finally {
  ...
  const selectUser = useCallback(async (u: User) => {
    ...
    try {
      const [data, cat] = await Promise.all([
        api.getUserSubscription(u.id).catch(() => null),
        api.getUserCatalogSubscription(u.id).catch(() => null),
      ]);
      setSubInfo(data as SubInfo | null);
      setCatalogBundle(cat as typeof catalogBundle);
    } catch {
      setSubInfo(null);
      setCatalogBundle(null);
    } finally {
  ...
  const grantSubscription = async () => {
    ...
    setMessage(null);
    try {
      await api.grantSubscription(selectedUser.id, { duration_days: days, plan: plan.label, note });
      setMessage({ type: 'ok', text: `✅ Abonnement "${plan.label}" accordé pour ${days} jours.` });
      // Refresh sub info
      const data = await api.getUserSubscription(selectedUser.id) as SubInfo;
      setSubInfo(data);
      setNote('');
    } catch (e: any) {
  ...
  const grantCatalogSubscription = async () => {
    ...
    setMessage(null);
    try {
      await api.grantCatalogPlan(selectedUser.id, {
      ...
      const cat = await api.getUserCatalogSubscription(selectedUser.id);
      setCatalogBundle(cat as typeof catalogBundle);
      setNote('');
    } catch (e: any) {
  ...
  const revokeSubscription = async () => {
    ...
    setSubmitting(true);
    try {
      await api.revokeSubscription(selectedUser.id);
