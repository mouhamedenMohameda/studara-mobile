...
export default function ResourcesPage() {
  ...
  const fetchScrapeStatus = async () => {
    try { setScrape(await api.bulkScrapeStatus()); } catch { /* ignore */ }
  };
  ...
  const handleBulkStart = async () => {
    ...
    try { await api.bulkScrapeStart(s, e); await fetchScrapeStatus(); }
    catch (err: any) { alert(err.message); }
  };
  const handleBulkStop = async () => {
    try { await api.bulkScrapeStop(); await fetchScrapeStatus(); } catch { /* ignore */ }
  };
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await api.resources(status, page);
  ...
  const handleModerate = async (id: string, action: 'approve' | 'reject', reason?: string) => {
    try {
      await api.moderate(id, action, reason);
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    try { await api.deleteResource(id); load(); }
    catch (e: any) { alert(e.message); }
  };
